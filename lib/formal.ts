...
function arithmetic(input: string): Expression {
  ...
  if (tokens.join('') !== text) throw new Error('Use numbers, single-letter variables, +, −, ×, /, parentheses, and absolute-value bars.');
  let index = 0;
  function atom(depth: number): Expression {
    ...
    if (token === '+' || token === '-') {
      ...
      return token === '+' ? child : { ...child, lean: `(-${child.lean})`, constant: child.constant ? rational(-child.constant.n, child.constant.d) : null };
    }
    if (token === '(' || token === '|') {
      const child = sum(token === '|', depth + 1);
      if (tokens[index++] !== (token === '(' ? ')' : '|')) throw new Error('Close every parenthesis and absolute-value pair.');
      if (token === '(') return child;
      if (child.abs) throw new Error('Nested absolute values are outside this proof translator.');
      return { ...child, lean: `|${child.lean}|`, abs: 1, constant: child.constant ? rational(child.constant.n < 0n ? -child.constant.n : child.constant.n, child.constant.d) : null };
    }
    if (/^\d*\.?\d+$/.test(token || '')) {
      if (token.replace('.', '').length > 12) throw new Error('Use numbers with at most 12 digits.');
      const [whole, fraction = ''] = token.split('.');
      const value = rational(BigInt((whole || '0') + fraction), 10n ** BigInt(fraction.length));
      return { lean: value.d === 1n ? `(${value.n} : ℝ)` : `((${value.n} : ℝ) / ${value.d})`, constant: value, variables: new Set(), abs: 0 };
    }
    if (/^[a-z]$/.test(token || '')) return { lean: `v_${token}`, constant: null, variables: new Set([token]), abs: 0 };
  ...
  function binary(a: Expression, op: string, b: Expression): Expression {
    if (op === '/' && !b.constant) throw new Error('The Lean translator supports only constant denominators.');
    if (op === '/' && b.constant!.n === 0n) throw new Error('Division by zero is undefined; no proof will be generated.');
    if (op === '*' && !a.constant && !b.constant) throw new Error('The Lean translator currently supports linear expressions.');
    let constant: Rational | null = null;
    if (a.constant && b.constant) {
      const x = a.constant, y = b.constant;
      constant = op === '+' ? rational(x.n * y.d + y.n * x.d, x.d * y.d)
        : op === '-' ? rational(x.n * y.d - y.n * x.d, x.d * y.d)
        : op === '*' ? rational(x.n * y.n, x.d * y.d) : rational(x.n * y.d, x.d * y.n);
    }
    return { lean: `(${a.lean} ${op} ${b.lean})`, constant, variables: new Set([...a.variables, ...b.variables]), abs: a.abs + b.abs };
  }
  ...
  function sum(inAbs: boolean, depth: number): Expression {
    ...
    while (tokens[index] === '+' || tokens[index] === '-') left = binary(left, tokens[index++], product(inAbs, depth));
    return left;
  }
...
function join(parts: Proposition[], operator: string): Proposition {
  return {
    lean: parts.length === 1 ? parts[0].lean : `(${parts.map(p => p.lean).join(` ${operator} `)})`,
...
function proposition(raw: string): Proposition {
  ...
  return join(branches.map(branch => {
    const clauses = branch.split(/\band\b/);
    if (clauses.length > 8) throw new Error('Use at most eight joined conditions.');
    return join(clauses.map(clause => {
      const parts = clause.split(/(<=|>=|=|<|>)/);
      if (parts.length < 3 || parts.length > 7) throw new Error('Write an equation or inequality, with complete branches joined by “or” or “and”.');
      const expressions = parts.filter((_, i) => i % 2 === 0).map(arithmetic);
      return join(expressions.slice(1).map((right, i) => {
        const left = expressions[i];
        const op = ({ '<=': '≤', '>=': '≥' } as Record<string, string>)[parts[2 * i + 1]] || parts[2 * i + 1];
        return { lean: `(${left.lean} ${op} ${right.lean})`, variables: new Set([...left.variables, ...right.variables]), abs: left.abs + right.abs };
      }), '∧');
    }), '∧');
  }), '∨');
}
...
export function proofDeclaration(statement: string, name = 'algebra_claim'): string {
  ...
  intros
  simp only [abs_eq_ite] <;> (try split_ifs) <;> aesop (add safe (by linarith))

#print axioms ${name}
`;
}
export function buildProof(request: ProofRequest): ProofClaim {
  if (!request || typeof request.before !== 'string' || typeof request.after !== 'string') throw new Error('Supply a starting relationship and a proposed result.');
  if (request.kind && !['step', 'solution'].includes(request.kind)) throw new Error('Choose a step or complete-solution proof.');
  if (request.constraints && (!Array.isArray(request.constraints) || request.constraints.length > 8 || request.constraints.some(c => typeof c !== 'string'))) throw new Error('Supply at most eight written constraints.');
  ...
  const after = join([proposition(request.after), ...(request.kind === 'step' ? constraints : [])], '∧');
  const variables = [...new Set([...before.variables, ...after.variables])].sort();
  if (variables.length > 4 || before.abs + after.abs > 8) throw new Error('Use at most four variables and eight absolute-value occurrences in a proof.');
