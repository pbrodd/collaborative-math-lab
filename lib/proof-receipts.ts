...
export function findReceipt(
  claim: ProofClaim,
  digest: string,
  report: ProofReport = bundled,
): ProofReceipt | undefined {
  if (
    report.format !== PROOF_FORMAT ||
    report.lean !== LEAN_VERSION ||
    report.mathlib !== MATHLIB_REVISION
  )
    return undefined;
  return report.claims.find(
    (r) =>
      r.digest === digest &&
      r.statement === claim.statement &&
      Array.isArray(r.axioms) &&
      r.axioms.every((a) => ['propext', 'Classical.choice', 'Quot.sound'].includes(a)),
  );
}
